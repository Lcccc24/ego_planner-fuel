...
int fuel_stop_flag = 0;
int fuel_lastpointrev_flag = 0;
// 标志flag 令fuel端不再往PX4发送控制指令
std_msgs::Int32 ego_start_flag;
// 接收到fuel端最后悬停点坐标
float fuel_lastpoint_x = 0.0f, fuel_lastpoint_y = 0.0f, fuel_lastpoint_z = 0.0f;
ros::Publisher ego_start_pub;
...
Ctrl::Ctrl() {
  ...
  &Ctrl::position_cb, this); target_sub =
  ...
  pubMarker = nh.advertise<visualization_msgs::Marker>("/track_drone_point",
  5);
...
void Ctrl::control(const ros::TimerEvent&) {
  if (!receive)  // 如果没有在rviz上打点，则offboard模式下会保持在1m的高度
  {
    ...
    current_goal.position.x = fuel_lastpoint_x;
    current_goal.position.y = fuel_lastpoint_y;
    current_goal.position.z = fuel_lastpoint_z;
    current_goal.yaw = now_yaw;
    ROS_INFO("请等待\n");
    ego_start_flag.data = 1;
    ego_start_pub.publish(ego_start_flag);
  }

  // if receive plan in rviz, the EGO plan information can input mavros and
  // vehicle can auto navigation
...
void fuel_stop_callback(const std_msgs::Int32::ConstPtr& msg) {
  fuel_stop_flag = msg->data;
}

void fuel_lastpoint_callback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
  fuel_lastpoint_x = msg->pose.position.x;
  fuel_lastpoint_y = msg->pose.position.y;
  fuel_lastpoint_z = msg->pose.position.z;
  fuel_lastpointrev_flag = 1;
}
...
int main(int argc, char** argv) {
  ...
  ego_start_pub = n.advertise<std_msgs::Int32>("/ego/start_flag", 10);
  ros::Subscriber fuel_stop_sub =
      n.subscribe("/fuel/stop_flag", 10, fuel_stop_callback);
  ros::Subscriber fuel_lastpoint_sub =
      n.subscribe("/fuel/hover_position", 10, fuel_lastpoint_callback);

  while (fuel_stop_flag == 0 || fuel_lastpointrev_flag == 0) {
    loop_rate.sleep();
    ros::spinOnce();
  }
