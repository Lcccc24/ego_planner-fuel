...
class Ctrl {
 public:
  ...
  std::vector<Target_Point> ego_target_points;
...
Ctrl::Ctrl() {
  ...
  ego_points_index = 0;
  // ego目标点坐标姿态
  ego_target_points = {{4.0, -1.35, 1.0},
                       {6.0, -0.8, 1.0},
                       {7.0, 4.0, 1.0},
                       {7.0, 6.0, 1.0},
                       {0.0, 0.0, 1.0}};
    ...
    if (armed) {
      switch (mission_flag) {
        case 0:
          ...
          if (abs(position_z -  current_goal.position.z) < 0.1f)
            mission_flag = 1;
      ...
      if (mission_flag == 1 || mission_flag == 5) {
        ...
        ROS_INFO("已触发控制器，当前EGO规划速度：vel_x,vel_z = %.2f,%.2f\n",
