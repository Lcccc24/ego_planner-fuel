...
void DataManager::spin() {
  ...
  takeoff_state.pos = Eigen::Vector3d(0, 0, 1.5);  // 原：odom_.pos;
...
void DataManager::takeoff() {
  ...
  data_input_->getUavState(cur_state);
  UavState takeoff_state;
  takeoff_state.pos = Eigen::Vector3d(0, 0, 1.5);  // 原：Eigen::Vector3d(odom_.pos(0), odom_.pos(1), 1.5);
  takeoff_state.yaw = 0;
  data_output_->sendDesireState(takeoff_state);
  if (SIM_OR_REAL == 0) {
  ...
  if ((odom_.pos - takeoff_state.pos).norm() < 0.3) {
    ROS_INFO("起飞成功");
