...
class DataRosOutput {
 public:
  ...
  void init();
 //void sendDesireState(const UavState &desire_state, const UavState &now_odom);
  void sendDesireState(const UavState &desire_state);
  void sendDesireVel(const UavState &desire_state);
  ...
  void publishPositionYaw(const mavros_msgs::PositionTarget &data) {
    target_pub_.publish(data);
  }

  // void publishVel(const geometry_msgs::TwistStamped &data){
